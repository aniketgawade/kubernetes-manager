...
class VncService(VncCommon):
    ...
    def _update_service_public_ip(self, service_id, service_name,
        ...
        if service_type in ["LoadBalancer"]:
            if len(allocated_fips) is 0:
                ...
                if loadBalancerIp:
                    allocated_fip = self._allocate_floating_ips(service_id, specified_fip_pool_fq_name_str, set([loadBalancerIp]))
                    if allocated_fip:
                        self._update_service_external_ip(service_namespace, service_name, allocated_fip)
                elif external_ips:
                    allocated_fips = self._allocate_floating_ips(service_id, specified_fip_pool_fq_name_str, external_ips)
                else:
                    allocated_fip = self._allocate_floating_ips(service_id, specified_fip_pool_fq_name_str)
                    if allocated_fip:
                        self._update_service_external_ip(service_namespace, service_name, allocated_fip)
            ...
            if len(allocated_fips):
                if loadBalancerIp and loadBalancerIp in allocated_fips:
                    ...
                    self._allocate_floating_ips(service_id, specified_fip_pool_fq_name_str, set([loadBalancerIp]))
                ...
                if external_ips and external_ips != allocated_fips:
                    ...
                    self._deallocate_floating_ips(service_id)
                    self._allocate_floating_ips(service_id, specified_fip_pool_fq_name_str, external_ips)
                    return
    ...
    def process(self, event):
        ...
        if annotations:
	    if 'opencontrail.org/fip_pool' in annotations:
	        specified_fip_pool_fq_name_str = annotations['opencontrail.org/fip_pool']
